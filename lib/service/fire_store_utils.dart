...
class FireStoreUtils {
  ...
  static Future<void> getSettings() async {
    try {
      ...
      // FIXED: Changed from .listen() to .get() - prevents memory leak from active listeners
      final driverNearBySnap1 =
          await fireStore
              .collection(CollectionName.settings)
              .doc("DriverNearBy")
              .get();
      if (driverNearBySnap1.exists && driverNearBySnap1.data() != null) {
        Constant.distanceType =
            driverNearBySnap1.data()?["distanceType"] ?? "km";
        Constant.isEnableOTPTripStart =
            driverNearBySnap1.data()?["enableOTPTripStart"] ?? false;
        Constant.isEnableOTPTripStartForRental =
            driverNearBySnap1.data()?["enableOTPTripStartForRental"] ?? false;
      }

      // FIXED: Changed from .listen() to .get() - prevents memory leak
      final maintenanceSnap =
          await fireStore
              .collection(CollectionName.settings)
              .doc("maintenance_settings")
              .get();
      if (maintenanceSnap.exists && maintenanceSnap.data() != null) {
        Constant.isMaintenanceModeForCustomer =
            maintenanceSnap.data()?["isMaintenanceModeForCustomer"] ?? false;
      }
      ...
      // FIXED: Changed from .listen() to .get() - prevents memory leak
      final googleMapKeySnap =
          await fireStore
              .collection(CollectionName.settings)
              .doc("googleMapKey")
              .get();
      if (googleMapKeySnap.exists && googleMapKeySnap.data() != null) {
        Constant.mapAPIKey = googleMapKeySnap.data()?["key"] ?? "";
      }

      // FIXED: Changed from .listen() to .get() - prevents memory leak
      final placeHolderSnap =
          await fireStore
              .collection(CollectionName.settings)
              .doc("placeHolderImage")
              .get();
      if (placeHolderSnap.exists && placeHolderSnap.data() != null) {
        Constant.placeHolderImage = placeHolderSnap.data()?["image"] ?? "";
      }

      // FIXED: Changed from .listen() to .get() - prevents memory leak
      final notificationSettingSnap =
          await fireStore
              .collection(CollectionName.settings)
              .doc("notification_setting")
              .get();
      if (notificationSettingSnap.exists) {
        Constant.senderId = notificationSettingSnap.data()?["senderId"];
        Constant.jsonNotificationFileURL =
            notificationSettingSnap.data()?["serviceJson"];
      }
      ...
      // FIXED: Changed from .listen() to .get() - prevents memory leak
      final privacyPolicySnap =
          await fireStore
              .collection(CollectionName.settings)
              .doc("privacyPolicy")
              .get();
      if (privacyPolicySnap.exists && privacyPolicySnap.data() != null) {
        Constant.privacyPolicy =
            privacyPolicySnap.data()?["privacy_policy"] ?? "";
      }
      ...
      final termsSnap =
          await fireStore
              ...
              .get();
      if (termsSnap.exists && termsSnap.data() != null) {
        Constant.termsAndConditions =
            termsSnap.data()?["termsAndConditions"] ?? "";
      }

      // FIXED: Changed from .listen() to .get() - prevents memory leak
      final walletSettingsSnap =
          await fireStore
              .collection(CollectionName.settings)
              .doc("walletSettings")
              .get();
      if (walletSettingsSnap.exists && walletSettingsSnap.data() != null) {
        Constant.walletSetting =
            walletSettingsSnap.data()?["isEnabled"] ?? false;
      }
      ...
      if (versionSnap.exists && versionSnap.data() != null) {
        Constant.googlePlayLink = versionSnap.data()?["googlePlayLink"] ?? '';
        Constant.appStoreLink = versionSnap.data()?["appStoreLink"] ?? '';
        Constant.appVersion = versionSnap.data()?["app_version"] ?? '';
        Constant.websiteUrl = versionSnap.data()?["websiteUrl"] ?? '';
      }
