...
class RestaurantDetailsController extends GetxController {
  ...
  // ✅ NEW: Timer for image carousel - properly managed
  Timer? _sliderTimer;
  ...
  void animateSlider() {
    if (vendorModel.value.photos != null &&
      ...
      // ✅ FIX: Cancel previous timer before creating new one
      _sliderTimer?.cancel();
  ...
  Future<void> getArgument() async {
    // ✅ FIX: Properly manage CartProvider stream subscription
  ...
  void searchProduct(String name) {
    List<ProductModel> filtered = allProductList;
    if (name.isNotEmpty) {
      filtered =
          filtered
              .where(
    ...
    if (selectedCategoryId.value.isNotEmpty) {
      filtered =
          filtered
              .where((p0) => p0.categoryID == selectedCategoryId.value)
              .toList();
    }
  ...
  void filterRecord() {
    ...
    if (isVag.value == true && isNonVag.value == true) {
      ...
      filtered = filtered.where((p0) => p0.nonveg == false).toList();
    } else if (isVag.value == false && isNonVag.value == true) {
      filtered = filtered.where((p0) => p0.nonveg == true).toList();
  ...
  void onClose() {
    // Cancel timer
    _sliderTimer?.cancel();
    // Cancel cart stream subscription
    _cartStreamSubscription?.cancel();
    // Dispose controllers
